...
- **Runs to completion** — context condensation lets big multi-file tasks finish in a single call, a stalled agent is nudged back into action instead of quietly giving up a step short of the finish, and a task running low on budget is steered to deliver a partial result rather than getting cut off with nothing.
